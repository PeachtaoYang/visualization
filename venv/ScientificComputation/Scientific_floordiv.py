...
class EmptyDelegate(QItemDelegate):
    ...
    def createEditor(self, QWidget, QStyleOptionViewItem, QModelIndex):
        return None
class myFloordiv(QDialog,Ui_Dialog):
    ...
    def __init__(self,df):
        ...
        self.setWindowTitle('（右）地板除法')
        self.setWhatsThis('右地板除法，即截断除法，不取余数，结果为整数\n'
                          '如： 原有数据 / 你输入的数据 ')
        ...
        if self.Data:
            ...
            for i in self.Data:
                if not isNumbertic(i[1]):
                    QMessageBox.information(self,'waring','数值类型不能与非数值类型相除！\n请仔细检查输入参数。',
